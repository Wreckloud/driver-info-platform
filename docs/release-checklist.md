...
- [ ] 成功页将定位结果合并到“起始位置”：成功时优先显示文字地址，失败时只显示可识别的失败原因，不向司机展示经纬度和精度。
...
- [ ] 登记成功页、管理员手机列表和详情中的姓名、手机号、车牌号、车型均按两列两行显示；成功页顶部将勾选图标、登记成功和服务器发车时间合并展示，出车照片位于起始位置下方并可横向滑动、点击放大；常规短内容在手机端尽量单页完整展示。
