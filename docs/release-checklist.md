...
- [ ] 手机端四个常用信息字段按两列两行显示；照片在上、定位在下且共用一个卡片，没有横向溢出。
- [ ] 至少完成一次拒绝定位或定位失败登记，两者均能正常提交。
- [ ] 成功页优先显示文字地址，不向司机展示经纬度和精度。
- [ ] 项目、数量和可选备注可正常提交、回显、修改、搜索及导出，“发车时间”由服务器提交时间生成。
- [ ] 自动定位会在进入页面时触发，失败后点击定位卡片可重试；每次点击拍摄 1 张照片，最新照片置前，1–9 张照片以固定尺寸横向滑动，可压缩、预览、提交并在后台放大查看。
- [ ] 提交前核对弹窗居中完整展示，四个常用信息字段按两列两行排列，320×667 屏幕下没有横向溢出或弹窗竖向滚动条。
- [ ] 登记成功页的四项简要信息、管理员手机列表和详情的四项常用信息均按两列两行显示，长信息保持整行。
- [ ] 只读账号只能查看与导出，修改和删除接口返回 403。
- [ ] 管理员完成登录、查询、日期筛选、编辑、软删除和当前筛选结果导出。
- [ ] 宽屏详情页的信息与定位时间卡片等高且不跟随滚动，出车照片位于两张卡片下方。
