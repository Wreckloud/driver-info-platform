...
`SUCCESS` 必须提供四个定位字段；`DENIED`、`FAILED`、`TIMEOUT`、`NOT_REQUESTED` 必须将定位字段省略或设为 `null`。文字地址和提交时间由后端生成。
...
- `PUT /api/admin/records/{id}`：只接收 `driverName/phone/licensePlate/vehicleType/destination`。
- `DELETE /api/admin/records/{id}`：软删除。
- `GET /api/admin/records/export`：使用与列表相同的日期和关键词条件，返回 `.xlsx`。

日期条件按上海时区的完整自然日解释。所有正常查询均排除软删除记录。
